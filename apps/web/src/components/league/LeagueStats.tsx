...
type CompleteView = "passing" | "rushing" | "receiving" | "blocking" | "tackling" | "scoring" | "player-special" | "team-total" | "team-passing" | "team-rushing" | "team-defense" | "team-sacks" | "team-turnovers" | "special-teams" | null;
...
const RUSHING_COLUMNS: StatColumn[] = [
  ...
  { label: "BRK TKL", fields: ["BrokenTackle", "Broken Tackle", "Broken Tackles"] }, { label: "JKE", fields: ["Juke", "Jukes"] },
];
const PASSING_COLUMNS: StatColumn[] = [
  { label: "GP", fields: ["GP", "Games Played", "Games"] }, { label: "CMP", fields: ["Completions", "Pass Completions", "CMP"] }, { label: "ATT", fields: ["Passing Attempts", "Pass Attempts", "ATT"] },
  { label: "CMP%", fields: ["Completion Percentage"], value: (row) => { const attempts = statValue(row, ["Passing Attempts", "Pass Attempts", "ATT"]); return attempts ? statValue(row, ["Completions", "Pass Completions", "CMP"]) / attempts * 100 : 0; }, display: (row) => `${columnValue(row, PASSING_COLUMNS[3]).toFixed(1)}%` },
  { label: "YDS", fields: ["Passing Yards", "Pass Yards", "PassYards"] }, { label: "AVG", fields: ["Passing Average", "Yards Per Attempt"] }, { label: "YDS/G", fields: ["Passing Yards Per Game"] }, { label: "LNG", fields: ["Passing Long", "Pass Long", "LNG"] }, { label: "TD", fields: ["Passing TD", "Pass TD", "TD Passes"] }, { label: "INT", fields: ["Interceptions Thrown", "Pass INT"] }, { label: "SACK", fields: ["Sacked", "Times Sacked"] }, { label: "SYL", fields: ["Sack Yards Lost", "SYL"] }, { label: "QBR", fields: ["QBR"] }, { label: "RTG", fields: ["Passer Rating", "Rating", "RTG"] },
];
const RECEIVING_COLUMNS: StatColumn[] = [
  { label: "GP", fields: ["GP", "Games Played", "Games"] }, { label: "REC", fields: ["Receptions", "REC"] }, { label: "TGTS", fields: ["Targets", "Receiving Targets"] }, { label: "YDS", fields: ["Receiving Yards", "Rec Yards", "RecYards"] }, { label: "AVG", fields: ["Receiving Average", "Yards Per Reception"] }, { label: "TD", fields: ["Receiving TD", "Rec TD"] }, { label: "LNG", fields: ["Receiving Long", "Rec Long"] }, { label: "BIG", fields: ["Big Receptions", "BIG"] }, { label: "YDS/G", fields: ["Receiving Yards Per Game"] }, { label: "FUM", fields: ["Fum", "Fumbles"] }, { label: "LST", fields: ["Fum Lost", "Fumbles Lost"] }, { label: "YAC", fields: ["Yards After Catch", "YAC"] }, { label: "FD", fields: ["Receiving First Downs", "Rec First Downs", "First Down"] },
];
...
const DEFENSIVE_COLUMNS: StatColumn[] = [
  { label: "GP", fields: ["GP", "Games Played", "Games"] },
  { label: "TKL", fields: ["Tackles", "Total Tackles", "TOT", "TKL"] },
  { label: "SACK", fields: ["Sacks", "Sack", "SACK"] },
  { label: "YDS", fields: ["Sack Yards", "Sack Yards Lost", "SackYards", "SackYardsLost", "SYL"] },
  ...
  { label: "YDS (INT)", fields: ["Int Ret Yds", "Interception Return Yards", "IntReturnYards"] },
  { label: "TD (INT)", fields: ["Int TD", "Interception TD", "Interception Return TD", "IntReturnTD"] },
  { label: "FF", fields: ["FF", "Forced Fumbles", "ForcedFumbles"] },
  { label: "FR", fields: ["FR", "Fumble Recoveries", "FumbleRecoveries"] },
  { label: "FTD", fields: ["Fum TD", "Fumble TD", "Fumble Return TD", "FumbleReturnTD"] },
  { label: "DL PLAYS", fields: [], value: (row) => statValue(row, DL_WIN_FIELDS) + statValue(row, DL_LOSS_FIELDS), display: (row) => String(statValue(row, DL_WIN_FIELDS) + statValue(row, DL_LOSS_FIELDS)) },
  { label: "WIN%", fields: DL_WIN_RATE_FIELDS, value: (row) => lineWinRate(row, DL_WIN_FIELDS, DL_LOSS_FIELDS, DL_WIN_RATE_FIELDS), display: (row) => `${lineWinRate(row, DL_WIN_FIELDS, DL_LOSS_FIELDS, DL_WIN_RATE_FIELDS).toFixed(1)}%` },
];
...
function PlayerStatsProfile({ player, row, team, stats, onBack }: { player: Player; row?: PlayerStats; team?: LeagueTeam; stats: PlayerStats[]; onBack: () => void }) {
  ...
  const gameTabs = isDefensive ? ["Defense"] as const : isQuarterback ? ["Passing", "Rushing"] as const : isReceiver ? ["Receiving", "Rushing", "Fumbles"] as const : ["Rushing", "Receiving", "Fumbles"] as const;
  ...
  return <main className="player-detail-page">
    ...
    <section className="player-detail-hero">
      ...
      <div className="player-detail-name"><span>{shownPosition || "PLAYER"}</span><h1>{name}</h1><p>{logo && <img src={logo} alt="" />} <b>{teamName(team || {})}</b> · {shownPosition || "--"}</p><button className="player-stat-side-toggle" type="button" onClick={() => setStatSide(statSide === "offense" ? "defense" : "offense")}>View {otherSide} Stats</button></div>
      <dl className="player-detail-facts"><div><dt>SIZE</dt><dd>{String(player.Size || "—")}</dd></div><div><dt>TEAM</dt><dd>{teamName(team || {})}</dd></div><div><dt>POSITION</dt><dd>{shownPosition || "—"}</dd></div><div><dt>STATUS</dt><dd><i /> Active</dd></div></dl>
    </section>
...
function TeamCompleteTable({ mode, rows, onBack, onMode, onTeam }: { mode: "total" | "passing" | "rushing" | "defense" | "sacks" | "turnovers" | "special"; rows: TeamTotal[]; onBack: () => void; onMode: (mode: CompleteView) => void; onTeam: (team: LeagueTeam) => void }) {
  const [sort, setSort] = useState<CompleteSort>({ column: "stat-0", direction: "desc" });
  const defensive = mode === "defense" || mode === "sacks";
  const special = mode === "special";
  const groups = mode === "turnovers" ? [{ label: "", columns: ["DIFF"] }, { label: "TAKEAWAYS", columns: ["INT", "FUM", "TOTAL"] }, { label: "GIVEAWAYS", columns: ["INT", "FUM", "TOTAL"] }]
    : special ? [{ label: "KICKOFFS", columns: ["ATT", "YDS", "AVG", "LNG", "TD"] }, { label: "PUNTS", columns: ["ATT", "YDS", "AVG", "LNG", "TD", "FC"] }]
    : [{ label: "TOTAL", columns: ["YDS", "YDS/G"] }, { label: "PASSING", columns: ["YDS", "YDS/G"] }, { label: "RUSHING", columns: ["YDS", "YDS/G"] }, { label: "POINTS", columns: ["PTS", "PTS/G"] }];
  const values = (row: TeamTotal) => {
    const sum = (fields: string[]) => row.rows.reduce((n, stat) => n + statValue(stat, fields), 0);
    if (special) {
      const kickAtt = sum(["Kick Return Attempts", "Kickoff Return Attempts", "KR ATT", "KRT"]); const kickYds = sum(["Kick Return Yards", "Kickoff Return Yards", "KR YDS"]);
      const puntAtt = sum(["Punt Return Attempts", "PR ATT", "PRT"]); const puntYds = sum(["Punt Return Yards", "PR YDS"]);
      return [kickAtt, kickYds, kickAtt ? kickYds / kickAtt : 0, sum(["Kick Return Long", "KR LNG"]), sum(["Kick Return TD", "KR TD"]), puntAtt, puntYds, puntAtt ? puntYds / puntAtt : 0, sum(["Punt Return Long", "PR LNG"]), sum(["Punt Return TD", "PR TD"]), sum(["Punt Fair Catches", "Fair Catches", "FC"])];
    }
    if (mode === "turnovers") return [row.turnoverDiff, row.interceptions, row.fumbleRecoveries, row.takeaways, row.interceptionsThrown, row.fumblesLost, row.giveaways];
    if (defensive) return [row.allowed, row.allowed / row.gp, row.allowedPassing, row.allowedPassing / row.gp, row.allowedRushing, row.allowedRushing / row.gp, row.pointsAllowed, row.pointsAllowed / row.gp];
    return [row.total, row.total / row.gp, row.passing, row.passing / row.gp, row.rushing, row.rushing / row.gp, row.points, row.points / row.gp];
  };
  ...
  const sortedRows = [...rows].sort((a, b) => {
    const comparison = sort.column === "team"
      ? teamName(a.team).localeCompare(teamName(b.team))
      : sort.column === "gp"
        ? a.gp - b.gp
        : values(a)[number(sort.column.replace("stat-", ""))] - values(b)[number(sort.column.replace("stat-", ""))];
    return (sort.direction === "asc" ? comparison : -comparison) || teamName(a.team).localeCompare(teamName(b.team));
  });
  const sortHeader = (label: string, column: string) => <th aria-sort={sort.column === column ? sort.direction === "asc" ? "ascending" : "descending" : "none"}><button className="stat-sort-button" type="button" onClick={() => changeSort(column)}>{label} {sort.column === column && <span aria-hidden="true">{sort.direction === "desc" ? "↓" : "↑"}</span>}</button></th>;
...
export function LeagueStats({ teams, games = [], onTeam, requestedPlayer = null, onPlayerClose }: { teams: LeagueTeam[]; games?: LeagueGame[]; onTeam: (team: LeagueTeam) => void; requestedPlayer?: Player | null; onPlayerClose?: () => void }) {
  ...
  const teamTotals = useMemo<TeamTotal[]>(() => {
    ...
    return teams.map((team) => {
      ...
      const completedGames = games.filter((game) => String(game.Qtr).toUpperCase() === "FINAL" && [game.Home, game.Away].some((value) => keys.includes(normalized(value))));
      const opponents = completedGames
      ...
      const interceptionsThrown = sum(["Interceptions Thrown", "Pass INT"]);
      const fumblesLost = sum(["Fum Lost", "Fumbles Lost"]);
      const takeaways = interceptions + fumbleRecoveries;
      const giveaways = interceptionsThrown + fumblesLost;
      const points = completedGames.reduce((total, game) => total + number(keys.includes(normalized(game.Home)) ? game.HomeScore : game.AwayScore), 0) || teamField(team, ["PF", "Points For", "Points"]);
      const pointsAllowed = completedGames.reduce((total, game) => total + number(keys.includes(normalized(game.Home)) ? game.AwayScore : game.HomeScore), 0) || teamField(team, ["PA", "Points Against", "Points Allowed"]);
      return { team, rows, gp: completedGames.length || teamGames(team), passing, rushing, total: passing + rushing, points, pointsAllowed, sacks: sum(["Sacks", "Sack"]), interceptions, fumbleRecoveries, takeaways, interceptionsThrown, fumblesLost, giveaways, turnoverDiff: takeaways - giveaways, allowedPassing, allowedRushing, allowed: allowedPassing + allowedRushing };
    });
  ...
  const teamLeadersFor = (category: Category): Leader[] => teamTotals.map((row) => {
    const key = category.fields[0] as "total" | "passing" | "rushing" | "sacks" | "turnoverDiff" | "allowed";
  ...
  const completeColumns: StatColumn[] = completeView === "passing" ? PASSING_COLUMNS : completeView === "rushing" ? RUSHING_COLUMNS : completeView === "receiving" ? RECEIVING_COLUMNS : completeView === "blocking" ? BLOCKING_COLUMNS : DEFENSIVE_COLUMNS;
  const isOffenseComplete = completeView === "passing" || completeView === "rushing" || completeView === "receiving" || completeView === "blocking";
  const completeRows = stats.filter((row) => {
    if (filter && !normalized(row.Player).includes(normalized(filter))) return false;
    return completeColumns.slice(1).some((column) => columnValue(row, column) > 0);
  }).sort((a, b) => {
    if (completeSort.column === "RK") {
      const entryColumn = completeColumns.find((candidate) => candidate.label === entrySortColumn) || completeColumns[0];
      const comparison = columnValue(a, entryColumn) - columnValue(b, entryColumn);
    ...
    if (completeSort.column === "POS") {
      const position = (row: PlayerStats) => completeView === "tackling" ? playerByName.get(normalized(row.Player))?.DefPos : playerByName.get(normalized(row.Player))?.Pos;
      const comparison = String(position(a) || "").localeCompare(String(position(b) || ""));
      return (completeSort.direction === "asc" ? comparison : -comparison) || String(a.Player).localeCompare(String(b.Player));
    }
  ...
  const openCompleteView = (view: Exclude<CompleteView, null>, column: string) => { setEntrySortColumn(column); setCompleteSort({ column, direction: "desc" }); setCompleteView(view); };
  const teamMode = completeView === "special-teams" ? "special" : completeView?.startsWith("team-") ? completeView.slice(5) as "total" | "passing" | "rushing" | "defense" | "sacks" | "turnovers" : null;
  ...
  return <main className="league-stats-card">
    <header className="league-stats-heading"><div><span>LEAGUE STATISTICS</span><h1>{teamMode ? `AFL Team ${teamMode === "total" ? "Total Offense" : teamMode === "defense" ? "Total Defense" : teamMode === "special" ? "Returning" : teamMode[0].toUpperCase() + teamMode.slice(1)} Stats` : completeTitle}</h1></div><StatsSelect label="Statistics view" value={activeView} onChange={(value) => { setActiveView(value as StatsView); setCompleteView(null); }}><option value="Player">Player Statistics</option><option value="Team">Team Statistics</option></StatsSelect></header>
    ...
    {!teamMode && <div className="season-row"><StatsSelect label="Season" value="season-1"><option value="season-1">Season 1 Regular Season</option></StatsSelect></div>}
    {teamMode ? <TeamCompleteTable mode={teamMode} rows={[...teamTotals].sort((a, b) => teamMode === "defense" ? a.allowed - b.allowed : teamMode === "turnovers" ? b.turnoverDiff - a.turnoverDiff : teamMode === "special" ? b.rows.reduce((n, stat) => n + statValue(stat, ["Kick Return Yards", "Kickoff Return Yards", "KR YDS"]), 0) - a.rows.reduce((n, stat) => n + statValue(stat, ["Kick Return Yards", "Kickoff Return Yards", "KR YDS"]), 0) : b.total - a.total)} onBack={() => setCompleteView(null)} onMode={setCompleteView} onTeam={onTeam} /> : completeView ? <section className="complete-rushing-leaders"><div className="player-stat-nav" role="tablist" aria-label="Player statistic category">{(["Offense", "Defense", "Scoring", "Special Teams"] as const).map((label) => { const active = label === "Offense" ? isOffenseComplete : label === "Defense" ? completeView === "tackling" : label === "Scoring" ? completeView === "scoring" : completeView === "player-special"; return <button role="tab" aria-selected={active} className={active ? "active" : ""} type="button" key={label} onClick={() => label === "Offense" ? openCompleteView("rushing", "YDS") : label === "Defense" ? openCompleteView("tackling", "TKL") : setCompleteView(label === "Scoring" ? "scoring" : "player-special")}>{label}</button>; })}</div>{isOffenseComplete || completeView === "tackling" ? <>{isOffenseComplete && <div className="offense-stat-nav" role="tablist" aria-label="Offensive statistic category">{(["passing", "rushing", "receiving", "blocking"] as const).map((view) => <button role="tab" aria-selected={completeView === view} className={completeView === view ? "active" : ""} type="button" key={view} onClick={() => openCompleteView(view, view === "blocking" ? "PLAYS" : "YDS")}>{view[0].toUpperCase() + view.slice(1)}</button>)}</div>}<div className="complete-leaders-tools"><button type="button" onClick={() => setCompleteView(null)}>← Back to stat leaders</button><label><span>Filter players</span><input type="search" value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Player name" /></label></div><div className="complete-leaders-table-scroll"><table className="complete-leaders-table"><thead><tr><th aria-sort={completeSort.column === "RK" ? completeSort.direction === "asc" ? "ascending" : "descending" : "none"}><button className="stat-sort-button" type="button" onClick={() => sortCompleteRows("RK")}>RK {completeSort.column === "RK" && <span aria-hidden="true">{completeSort.direction === "desc" ? "↓" : "↑"}</span>}</button></th><th aria-sort={completeSort.column === "Player" ? completeSort.direction === "asc" ? "ascending" : "descending" : "none"}><button className="stat-sort-button" type="button" onClick={() => sortCompleteRows("Player")}>NAME {completeSort.column === "Player" && <span aria-hidden="true">{completeSort.direction === "desc" ? "↓" : "↑"}</span>}</button></th><th aria-sort={completeSort.column === "POS" ? completeSort.direction === "asc" ? "ascending" : "descending" : "none"}><button className="stat-sort-button" type="button" onClick={() => sortCompleteRows("POS")}>POS {completeSort.column === "POS" && <span aria-hidden="true">{completeSort.direction === "desc" ? "↓" : "↑"}</span>}</button></th>{completeColumns.map((column) => <th key={column.label} aria-sort={completeSort.column === column.label ? completeSort.direction === "asc" ? "ascending" : "descending" : "none"}><button className="stat-sort-button" type="button" onClick={() => sortCompleteRows(column.label)}>{column.label} {completeSort.column === column.label && <span aria-hidden="true">{completeSort.direction === "desc" ? "↓" : "↑"}</span>}</button></th>)}</tr></thead><tbody>{completeRows.map((row, index) => { const player = playerByName.get(normalized(row.Player)); return <tr key={row.Player}><td>{index + 1}</td><td className="complete-leader-player"><PlayerImage player={player || {}} className="player-stats-image" /><button className="player-name-button" type="button" onClick={() => player && setSelectedPlayer(player)}>{row.Player}</button></td><td>{String((completeView === "tackling" ? player?.DefPos : player?.Pos) || "—")}</td>{completeColumns.map((column) => <td key={column.label}>{column.display?.(row) ?? displayStat(row, column.fields)}</td>)}</tr>; })}</tbody></table></div></> : <div className="player-stats-stub" role="status"><h2>{completeView === "scoring" ? "Scoring" : "Special Teams"}</h2><p>Player leaders for this category are coming soon.</p><button type="button" onClick={() => setCompleteView(null)}>← Back to stat leaders</button></div>}</section> : <div className="stats-leader-grid"><section><h2>Offensive Leaders</h2>{(activeView === "Team" ? TEAM_OFFENSE : OFFENSE).map((category) => <LeaderTable key={category.title} category={category} leaders={activeView === "Team" ? teamLeadersFor(category) : leadersFor(category)} loading={loading} error={error} onPlayer={setSelectedPlayer} onTeam={onTeam} onComplete={activeView === "Team" ? () => setCompleteView("team-total") : () => openCompleteView(category.title.toLowerCase() as "passing" | "rushing" | "receiving", "YDS")} />)}</section><section><h2>Defensive Leaders</h2>{(activeView === "Team" ? TEAM_DEFENSE : DEFENSE).map((category) => <LeaderTable key={category.title} category={category} leaders={activeView === "Team" ? teamLeadersFor(category) : leadersFor(category)} loading={loading} error={error} onPlayer={setSelectedPlayer} onTeam={onTeam} onComplete={activeView === "Team" ? () => setCompleteView(category.title === "TURNOVERS" ? "team-turnovers" : "team-defense") : () => openCompleteView("tackling", category.label === "TOT" ? "TKL" : category.label)} />)}</section></div>}
