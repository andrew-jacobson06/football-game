...
export function LeagueStats({ teams, games = [], onTeam }: { teams: LeagueTeam[]; games?: LeagueGame[]; onTeam: (team: LeagueTeam) => void }) {
  ...
  const completeRows = stats.filter((row) => {
    ...
    if (completeSort.column === "POS") {
      const comparison = String(playerByName.get(normalized(a.Player))?.Pos || "").localeCompare(String(playerByName.get(normalized(b.Player))?.Pos || ""));
  ...
  const completeTitle = isOffenseComplete ? `AFL Player ${completeView[0].toUpperCase() + completeView.slice(1)} Stats` : completeView ? "AFL Player Stat Leaders" : "AFL Stat Leaders";
